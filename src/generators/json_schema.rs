...
impl JsonSchemaGenerator {
            ...
            Schema::Object { 
                ...
                if let Some(props) = properties {
                    ...
                    for (key, prop_schema) in props {
                        properties_json.insert(key.clone(), self.schema_to_json_schema(prop_schema)?);
                    }
                    json_schema.insert("properties".to_string(), Value::Object(properties_json));
                    
                    if let Some(req) = required {
                        if !req.is_empty() {
                            json_schema.insert("required".to_string(), 
                ...
                // Handle array items
                if let Some(items_schema) = items {
                    json_schema.insert("items".to_string(), self.schema_to_json_schema(items_schema)?);
                }
                ...
                if let Some(nullable_val) = nullable {
                    if *nullable_val {
                        if let Some(existing_type) = json_schema.get("type") {
                            json_schema.insert("type".to_string(), 
                                json!([existing_type.clone(), "null"]));
                        }
                    }
                }
    ...
    fn generate_full_schema(&self, schema: &OpenApiSchema) -> Result<Value> {
        ...
        json_schema.insert("$schema".to_string(), 
                ...
                if !schemas.is_empty() {
                    ...
                    if let Some(root_schema) = schemas.get("Root") {
                        ...
                        if let Some(first_name) = schemas.keys().next() {
                            json_schema.insert("$ref".to_string(), 
...
impl Generator for JsonSchemaGenerator {
    ...
    fn generate_with_command(&self, schema: &OpenApiSchema, command: &str) -> Result<String> {
        ...
        output.push_str(&serde_json::to_string_pretty(&json_schema)?);
        Ok(output)
    }
}
