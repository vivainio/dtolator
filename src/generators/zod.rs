    ...
    fn generate(&self, schema: &OpenApiSchema) -> Result<String> {
        ...
        if let Some(components) = &schema.components {
            if let Some(schemas) = &components.schemas {
                // Filter out request types
                let filtered_schemas: IndexMap<String, Schema> = schemas
                    .iter()
                    .filter(|(name, _)| !request_types.contains(*name))
                    .map(|(name, schema_def)| (name.clone(), schema_def.clone()))
                    .collect();
                
                // Sort schemas topologically by dependencies
                let sorted_names = self.topological_sort(&filtered_schemas)?;
                
                // Generate schemas in dependency order
                for name in sorted_names {
                    if let Some(schema_def) = filtered_schemas.get(&name) {
