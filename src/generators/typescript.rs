                    ...
                    for name in sorted_names {
                        if let Some(schema_def) = schemas.get(&name) {
                            ...
                            if self.generate_models_in_separate_files {
                                interface.push_str(&format!("// FILE: {name}.ts\n"));
                            }
                            interface.push_str(&self.generate_interface(&name, schema_def)?);
...
impl Clone for TypeScriptGenerator {
    fn clone(&self) -> Self {
        Self {
            ...
            generate_models_in_separate_files: self.generate_models_in_separate_files,
        }
    }
}
