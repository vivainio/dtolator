...
impl PydanticGenerator {
        ...
        match schema {
            Schema::Object { 
                ...
                if let Some(all_of_schemas) = all_of {
                    ...
                    output.push_str(&format!("{}    # This model combines multiple schemas (allOf)\n", self.indent()));
                    
                    // Add fields from all schemas
                    for schema in all_of_schemas.iter() {
                        if let Schema::Object { properties: Some(props), required, .. } = schema {
                            for (prop_name, prop_schema) in props {
                                ...
                                if is_required {
                                    output.push_str(&format!("{}    {}: {}\n", 
                                        self.indent(), prop_name, base_field_type));
                                } else {
                                    ...
                                    if base_field_type.starts_with("Optional[") {
                                        ...
                                        output.push_str(&format!("{}    {}: Optional[{}] = None\n", 
                                            self.indent(), prop_name, base_field_type));
                                    }
                                }
                            }
                    ...
                    if let Some(props) = properties {
                        if props.is_empty() {
                            ...
                            for (prop_name, prop_schema) in props {
                                ...
                                // Handle nullable + optional case to avoid double Optional
                                let field_type = if !is_required && base_field_type.starts_with("Optional[") {
                                ...
                                let field_def = self.generate_field_definition(prop_name, prop_schema, field_type, is_required)?;
    ...
    fn generate_field_definition(&self, name: &str, schema: &Schema, field_type: String, is_required: bool) -> Result<String> {
        if let Schema::Object { 
            min_length, max_length, minimum, maximum, pattern, description, .. 
        } = schema {
            ...
            if is_required {
                if constraints.is_empty() {
                    ...
                    Ok(format!("{}: {} = Field({})", name, field_type, constraints.join(", ")))
                }
            } else {
                if constraints.is_empty() {
                    ...
                    // Check if field_type is already Optional to avoid double-wrapping
                    if field_type.starts_with("Optional[") {
                        Ok(format!("{}: {} = Field(None, {})", name, field_type, constraints.join(", ")))
                    } else {
                        Ok(format!("{}: Optional[{}] = Field(None, {})", name, field_type, constraints.join(", ")))
                    }
                }
            }
        } else {
...
impl Generator for PydanticGenerator {
    fn generate(&self, schema: &OpenApiSchema) -> Result<String> {
        ...
        output.push_str("# Generated Pydantic models from OpenAPI schema\n");
